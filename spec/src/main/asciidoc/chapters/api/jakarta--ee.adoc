...
Jakarta Query is a specification that defines a portable, persistence-oriented query language for Java applications. Its goal is to provide a unified query model that works across database engines — including relational and non-relational (NoSQL) databases.

Jakarta NoSQL builds on top of Jakarta Query Core as a minimum requirement. Every Jakarta NoSQL provider must support the Core language, which includes basic operations such as entity selection (`FROM`), filtering (`WHERE`), updates (`SET`), and deletions (`DELETE`). NoSQL databases may also implement additional capabilities from the extended Jakarta Query specification.

The level of support depends on the underlying database provider. For example, some NoSQL databases might not support all operations (like certain functions or update statements), while others may apply implicit query limits or work under eventual consistency. Despite these differences, Jakarta NoSQL ensures that the query structure remains consistent and portable — delegating execution to the provider while handling entity mapping and transformation transparently.
