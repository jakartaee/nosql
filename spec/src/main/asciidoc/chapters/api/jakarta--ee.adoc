...
*Jakarta Query* is a specification that defines an object-oriented, portable, and extensible query language for persistence technologies. While it is part of the Jakarta EE family, its design is not limited to relational databases. Instead, it is explicitly crafted to support a wide range of data stores, including NoSQL systems.

Jakarta NoSQL builds on top of *Jakarta Query Core* as a foundational requirement. This core language defines a minimal yet expressive subset of the query model — including entity-based selection, filtering, ordering, and basic update/delete semantics — that all Jakarta NoSQL providers must support. It provides a unified foundation for querying data in a type-safe and declarative manner, across key-value, document, column, and graph databases.

Although the syntax remains consistent, query behavior may vary based on the capabilities of the underlying NoSQL database. Examples include:

- Implicit limits or pagination imposed by the database engine.
- Eventual consistency or asynchronous persistence models.
- Partial or database-specific support for certain functions or expressions.

Jakarta NoSQL delegates query execution to the appropriate provider, handling the translation from the abstract query language to native database operations. This approach ensures that application code remains portable and expressive, while still allowing each NoSQL provider to optimize behavior according to its model and features.

By integrating Jakarta Query, Jakarta NoSQL enables developers to write clear, vendor-independent queries — while embracing the unique strengths and limitations of the NoSQL ecosystem.
