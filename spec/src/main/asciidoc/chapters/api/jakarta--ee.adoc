...
While Jakarta NoSQL offers a unified query interface based on Jakarta Query Core, the actual capabilities and behaviors can vary significantly depending on the underlying NoSQL database type. These limitations are inherent to how each database model handles data access, indexing, and consistency.

Below are key considerations by NoSQL category:

===== Key-Value Databases

Key-Value stores provide the most limited querying capability:

- Only equality (`=`) comparisons are typically supported on the primary key attribute (`jakarta.nosql.@Id`).
- Queries like `FROM Entity WHERE id = :id` are supported.
- DELETE operations behave similarly — only by `jakarta.nosql.@Id`.
- UPDATE operations are **not supported**.
- No support for filtering by other fields or complex expressions.

===== Wide-Column Databases

Slightly more expressive than Key-Value:

- Support for equality (`=`) on `jakarta.nosql.@Id` is standard.
- Some providers allow filtering on other fields (e.g., `>`, `<`, `IN`), but **require secondary indexes**, which may degrade performance.
- UPDATE and DELETE require `jakarta.nosql.@Id` and follow the same constraints as SELECT.
- Advanced conditions may or may not be supported depending on the database and index setup.

===== Document Databases

Document databases offer the most flexible query model:

- Support filtering across multiple fields, not just `jakarta.nosql.@Id`.
- Enable use of a wide range of operators like `=`, `>`, `<`, `IN`, `AND`, `OR`, etc.
- Typically support SELECT with `ORDER BY` and pagination-style features.
- Full support for UPDATE and DELETE operations.
- Better suited for expressive queries in Jakarta Query.

===== Graph Databases

Graph providers generally offer capabilities comparable to document databases:

- Allow deep filtering, traversals, and querying associated nodes/relationships.
- Full SELECT, UPDATE, and DELETE operations are often supported depending on the graph engine.

===== Other or Custom NoSQL Providers

If a NoSQL database does not fit clearly into any of the categories above, it must support **at least the capabilities defined by the Key-Value model** — that is, equality conditions on `jakarta.nosql.@Id` for SELECT and DELETE operations. Additional support is welcome and encouraged, as long as it respects Jakarta Query Core semantics.

===== Function and Time Expression Limitations

Many NoSQL databases — regardless of type — **do not natively support scalar or time-based functions** in their query engines. This includes operations such as:

- `LENGTH(...)`, `LOWER(...)`, `UPPER(...)`, `ABS(...)`
- Time-based comparisons like `NOW()`, `CURRENT_DATE`, etc.

Because these operations are typically executed at the database level, they may not be portable or reliable across NoSQL providers. Implementations may throw `UnsupportedOperationException` or defer evaluation to the application layer (if applicable).

==== Object Mapping Integration

Jakarta NoSQL integrates deeply with Jakarta Query by allowing query expressions to resolve against Java classes annotated with mapping metadata. This ensures consistency between the domain model and the query logic, while enabling database-specific providers to transform data automatically.

Key concepts:

*Attribute Resolution*

Query fields are matched to Java attributes based on the following annotations:
- `@jakarta.nosql.Id`: Marks the primary key of the entity.
- `@jakarta.nosql.Column`: Marks attributes that are persistable and eligible for query evaluation.

Only fields annotated with `@Id` or `@Column` are considered during query execution. Fields without these annotations are ignored for persistence and querying.

*Entity Resolution*

Classes or records involved in a query must be annotated with:
- `@jakarta.nosql.Entity`: Identifies a class as a persistable root entity.

These entity classes define the logical *FROM* target in a query. A query such as:

[source,sql]
----
FROM Product WHERE category = 'TECH'
----

...relies on the existence of a class like:
...
*Class Name Matching and Conflict Resolution*

Entity resolution is based on the **simple name** of the class (e.g., `Product`), not the fully qualified package. This means that if two classes exist with the same simple name but different packages, such as:

[source,java]
----
com.example.catalog.Product
com.partner.model.Product
----

...and both are annotated with `@Entity`, **the behavior is undefined**. Jakarta NoSQL delegates resolution to the provider, and the specification does not define what happens in case of naming conflicts.
