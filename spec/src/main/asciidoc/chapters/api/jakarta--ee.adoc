...
While Jakarta NoSQL offers a unified query interface based on Jakarta Query Core, the actual capabilities and behaviors can vary significantly depending on the underlying NoSQL database type. These limitations are inherent to how each database model handles data access and indexing.

Below are key considerations per NoSQL type:

*Key-Value Databases*

Key-Value databases revolve around direct access via keys, making their query capabilities very limited:
- Supports only equality (`=`) conditions on the identifier attribute (`@Id`).
- Queries such as `FROM Entity WHERE id = :id` are supported.
- Equivalent DELETE operations may work using the same logic.
- UPDATE operations are **not supported**.

*Wide-Column Databases*

These databases are similar to Key-Value in structure but may support additional operators depending on configuration:
- Basic support for equality (`=`) on `@Id`.
- Other fields and operators (e.g., `>`, `<`, `IN`) may be supported **only if a secondary index is created**, which can affect performance.
- UPDATE and DELETE follow the same constraint: as a minimum, the operation must include an equality check on the `@Id` field.
- Support for other filtering operations may be vendor-specific.

*Document Databases*

Document databases are generally more flexible:
- Queries can involve multiple attributes, not limited to `@Id`.
- Rich support for comparison operators like `=`, `>`, `<`, `IN`, etc.
- Supports ORDER BY and pagination-like behaviors (when available).
- UPDATE, DELETE, and SELECT operations are usually fully supported within the database’s capabilities.

*Graph Databases*

Graph databases share many of the same capabilities as Document stores in terms of query flexibility:
- Complex queries involving multiple fields and relationships are typically supported.
- Like documents, they support full SELECT, UPDATE, and DELETE semantics — though performance may depend on traversal and index strategy.

IMPORTANT: Regardless of query syntax, the actual support for operators, indexing behavior, implicit limits, and consistency guarantees (e.g., eventual consistency) may vary between providers.

Always consult your NoSQL provider’s documentation for query-specific behaviors and constraints.
