...
Additional features — such as filtering, sorting, or update operations — are allowed if they respect Jakarta Common Query Language (JCQL) semantics.
...
Jakarta NoSQL integrates query execution with Jakarta Mapping metadata. This integration allows Jakarta Common Query Language (JCQL) to resolve attribute names and types based on the annotated Java class structure.
