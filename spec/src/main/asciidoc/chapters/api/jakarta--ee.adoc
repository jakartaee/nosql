...
- DELETE operations follow the same constraints — only by `@Id`.
...
Since these are generally evaluated at the database level, behavior is not guaranteed and may vary by provider. Unsupported operations may result in:

- `UnsupportedOperationException`
- Evaluation deferred to the application layer (optional and non-portable)

==== Object Mapping Integration

Jakarta NoSQL integrates query execution with Jakarta Mapping metadata. This allows queries to resolve field names and types based on the annotated Java class structure.

Query engines match attributes and structures as follows:

- Classes must be annotated with `@jakarta.nosql.Entity`.
- Only fields or constructor parameters annotated with `@jakarta.nosql.Id` or `@jakarta.nosql.Column` are visible to the query engine.
- Fields without these annotations are not considered during query parsing or result transformation.

This mapping ensures consistency between domain models and query behavior.

TIP: If two entity classes share the same simple name but have different packages or qualified names, the result is undefined. Jakarta NoSQL does not define behavior for disambiguating entities with duplicate names. For clarity, use the `name` attribute in the `@Entity` annotation to assign a unique query name.
