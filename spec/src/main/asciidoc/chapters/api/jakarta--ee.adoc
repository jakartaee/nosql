...
Jakarta NoSQL builds upon the foundation provided by _Jakarta Query_, a portable, object-oriented query language designed to abstract common database operations.

All Jakarta NoSQL providers must support the **Jakarta Query Core** language, which enables structured access to NoSQL data using a consistent query syntax. This makes it possible to write expressive queries across a wide variety of underlying databases — from document to graph — without locking into database-specific syntax.

=== Supported Operations

The Jakarta Query Core language includes support for three core operations:

==== `SELECT`

Queries using `SELECT` retrieve entities from a data source, optionally filtered by conditions (`WHERE`) and sorted (`ORDER BY`).

The `FROM` clause specifies the entity to query. Filtering conditions may use comparison operators (`=`, `>`, `<=`, `<>`, etc.) and logical expressions (`AND`, `OR`, `NOT`). The `ORDER BY` clause defines how the result set is sorted.

NOTE: Some NoSQL providers may impose default result limits or ignore ordering unless explicitly supported.

==== `UPDATE`

Queries using `UPDATE` modify attributes of entities that match a condition.

The `SET` clause is used to assign new values to fields, while the `WHERE` clause narrows which records are affected.

IMPORTANT: Not all NoSQL databases support partial updates. In such cases, this operation may result in full entity replacement or raise an `UnsupportedOperationException`.

==== `DELETE`

Queries using `DELETE` remove entities matching the specified `WHERE` condition.

NOTE: As with `UPDATE`, delete operations may behave differently depending on database capabilities (e.g., soft vs. hard deletion, transactionality).

=== Limitations and Portability

While Jakarta Query provides a consistent query interface, its execution is **subject to the capabilities and constraints of the underlying NoSQL provider**. For example:

* Some databases may not support advanced filtering or dynamic sorting.
* Eventual consistency may result in observable delays between `UPDATE` or `DELETE` and subsequent `SELECT`.
* Certain expressions or query paths may be unsupported or require index definitions.

Jakarta NoSQL parses the query, maps it to the database-native format, and delegates execution to the provider — but database-specific limitations may still apply.

This approach promotes code portability while allowing flexibility for providers to optimize for their own engines.
