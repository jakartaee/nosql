...
As part of the Jakarta EE ecosystem, Jakarta NoSQL builds on top of *Jakarta Query Core* as a foundational requirement. This core language provides a minimal but expressive set of constructs — such as filtering, ordering, selection, and updates — that serve as the base for querying entities in a database-independent way.
