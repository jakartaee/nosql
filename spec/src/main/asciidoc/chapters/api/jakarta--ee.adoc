...
Query attributes are matched to Java attributes based on the following annotations:

- `@jakarta.nosql.Id`
- `@jakarta.nosql.Column`

Only attributes annotated with `@Id` or `@Column` are considered during query execution. Fields without these annotations are ignored for persistence and querying.
