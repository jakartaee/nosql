...
The `Template` class in Jakarta NoSQL simplifies CRUD (Create, Read, Update, Delete) operations by providing a fluent API for interacting with the underlying NoSQL database. This API allows developers to perform advanced queries, update and deletion operations beyond the basic ID attribute.
...
template.update(Book.class)
        .set("title")
        .to("Efective Java - Third Edition")
        .where("author")
        .eq("Joshua Bloch")
        .and("edition")
        .gt(3)
...
=== Query Function Expressions

Jakarta NoSQL supports function expressions in query conditions, aligning with the Jakarta Query specification. Function expressions allow scalar operations to be performed on entity fields within the fluent API.

==== Scalar Functions

Jakarta NoSQL provides several scalar functions for string and numeric operations:

===== String Functions

Example using the `UPPER` function for case-insensitive matching:
...
Example using the `LEFT` function to match the first characters:

[source,java]
----
List<Word> jaWords = template.select(Word.class)
        .where(Function.left("term", 2))
        .eq("Ja")
        .result();
----

Example using the `LENGTH` function to filter by string size:
...
Example using the `ABS` function for absolute value comparison:
...
Functions can be combined with logical operators such as `and` and `or` to create complex query conditions:
...
Jakarta NoSQL also supports ordering results by function expressions:
...
IMPORTANT: Function support varies significantly across NoSQL databases. Most NoSQL databases do not natively support scalar functions.

.Function Support by Database
[cols="1,1,1,1,1,1"]
|===
|Database |ABS |LENGTH |LOWER |UPPER |LEFT/RIGHT
...
When a function is not supported by the underlying database, an `UnsupportedFunctionException` will be thrown at query execution time:
...
try {
    ...
    // Fallback: fetch all and filter in application
