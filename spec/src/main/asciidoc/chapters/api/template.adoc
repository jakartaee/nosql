...
In this scenario, querying for records based on the zip code at the `MailingAddress` class requires accessing the `address` field at `Person` and the `zipcode` property at `MailingAddress`.
